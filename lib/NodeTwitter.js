...
module.exports = class NodeTwitter {
          ...
          .map((currentValue, index, array) => async () => {
            ...
            try {
              ...
              if (
                !originalError ||
                !originalError[0] ||
                originalError[0].code !== 162
              )
                throw e;
              console.log(
                chalk.red(currentValue + " blocked " + screenName + ".")
              );
            }
  ...
  async unfollow(params) {
    try {
      const { option, screenName } = params;
      if (option === "--after-error") {
        await unfollowAfterError({ screenName });
        return;
      } else if (option) {
        throw new Error(
          option + " is an unrecognized option. Did you mean --after-error?"
        );
      }
      const dir = getDirectory({ screenName });
      const client = await getClient({ dir, screenName });
...
async function listFriendsScreenNames(params) {
  try {
    ...
    if (jsonObj[0].next_cursor === 0)
      return friendsScreenNames.concat(
...
async function readLatestNotFollowingBack(params) {
  try {
    const { dir, screenName } = params;
    const fileID = objects.isDefined(params.fileID) ? params.fileID : 0;
    const file = dir + "/not-following-back." + fileID + ".json";
    if (!files.existsSync(file)) {
      const lastFile = dir + "/not-following-back." + (fileID - 1) + ".json";
      if (!files.existsSync(lastFile)) return [];
      const jsonStr = await files.readFile(lastFile, "utf8");
      return json.parse(jsonStr);
    }
    return await readLatestNotFollowingBack({
      dir,
      fileID: fileID + 1,
      screenName,
    });
  ...
  try {
    ...
    try {
      ...
      if (e[0] && e[0].code === 34) {
        ...
        console.log(chalk.red(user.screen_name + " does not exist."));
      } else if (
        e.message &&
        e.message.includes &&
        (e.message.includes("ENETDOWN") || e.message.includes("ENOTFOUND"))
      ) {
        // The following is an example stack trace of e:
        // Error: connect ENETDOWN 104.244.42.2:443 - Local (192.168.1.160:62601)
        // at internalConnect (net.js:919:16)
        // at defaultTriggerAsyncIdScope (internal/async_hooks.js:378:12)
        // at GetAddrInfoReqWrap.emitLookup [as callback] (net.js:1059:9)
        // at GetAddrInfoReqWrap.onlookup [as oncomplete] (dns.js:68:8)
        const maxRequestNum = 3;
        if (requestNum >= maxRequestNum - 1) {
          throw e;
        }
        console.log(chalk.red(e.message));
        console.log(
          "Waiting 1 minute to make another friendships/destroy request (" +
            (requestNum + 2) +
            " of " +
            maxRequestNum +
            ")..."
...
async function unfollowAfterError(params) {
  try {
    const { screenName } = params;
    const dir = getDirectory({ screenName });
    const client = await getClient({ dir, screenName });
  ...
  try {
    ...
    await notFollowingBack
      .map((user, index, array) => async () => {
        ...
        await unfollow({ client, dir, user });
      })
      // Execute each Promise one after the other.
      .reduce(
        (accumulator, currentValue) => accumulator.then(currentValue),
